...
3. Map `Case ID` to the case identifier, `Activity` to the activity, `Start Timestamp` to the start time, and `Complete Timestamp` to the complete time.
...
5. Use the duration view to distinguish arrival variability, activity duration, and the formula-check bottleneck. Use `disco_activity_summary.csv` to explain the human-owned visual/presentation review.
